...
contract_interface = contract_compiled['<stdin>:Transaction']
Transaction = w3.eth.contract(abi=contract_interface['abi'], 
                          bytecode=contract_interface['bin'])

# w3.eth.personal.unlockAccount(w3.eth.accounts[0], '') #  Not needed with Ganache
tx_hash = Transaction.constructor().transact({'from':w3.eth.accounts[0]})
tx_receipt = w3.eth.waitForTransactionReceipt(tx_hash)

# Contract Object
transaction = w3.eth.contract(address=tx_receipt.contractAddress, abi=contract_interface['abi'])
